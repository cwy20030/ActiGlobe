test_that("write.cosinor exports PDF and summary CSV correctly", {
  ...
  expect_true(all(c("MESOR", "Amplitude", "Acrophase",
                    "Acrophase.time") %in% names(out)))
  ...
  expect_false(all(c("MESOR", "Bathyphase.time", "Trough.ph",
                     "Acrophase.time", "Peak", "Amplitude") %in% names(out)))
  ...
  expect_true(all(c("MESOR", "Bathyphase.time", "Trough.ph", "Acrophase.time",
                    "Peak", "Amplitude") %in% names(out)))
})


test_that("write.cosinor handles method='KDE' correctly", {
  tmpdir <- tempdir()
  ID <- "TEST_KDE"
...
test_that("write.cosinor handles multiple tau values", {
  tmpdir <- tempdir()
  ID <- "TEST_MULTITAU"
...
test_that("write.cosinor handles default VAct and VTm", {
  tmpdir <- tempdir()
  ID <- "TEST_DEFAULT"
  ...
  # Test with NULL VAct and VTm (should use defaults)
  write.cosinor(
    Dir = tmpdir,
    ID = ID,
    DailyAct = dfList$Daily_df,
