...
#' `Incomplete Recording`), an exclusion flag, and the expected number of epochs
#' for a full day.
#'
#' @import hms
...
BriefSum <- function (df, SR, Start, TZ = "local") {
    ...
    Tm <- as.numeric (as.POSIXct (Start)) + (0:(nrow (df) - 1) * Epc)

    ### Convert date time
    AllT <-
        ifelse (is.numeric (Tm), as.POSIXct (x = Tm,
                                             origin = "1970-01-01",
                                             tz = TZ),
                                     as.POSIXct (x = Tm,
                                                 tz = TZ))
    ...
    ###### When daylight saving occurs, dynamically change UTCs
    ###### based on the dominant TZ.
    if (length (aTZs) > 1) aTZs$TZ3 <- ifelse (aTZs [[1]] > aTZs [[2]],
                                               names (aTZs) [[1]],
                                               names (aTZs) [[2]])
    ...
    df$nPoint <- seq_len (length.out = nrow (df))
