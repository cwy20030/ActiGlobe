...
#'
#' @description
#' Create visualization of a CosinorM or CosinorM.KDE fit using ggplot2. The
#' plot shows the parametric cosinor fit over a fine time grid, optional
#' pointwise confidence bands, observed data points, MESOR line, acrophase
#' verticals, amplitude annotation segments, and labelled parameter values when
#'  requested.
#'
...
#' @param labels Logical; Default `TRUE` places repelled labels on the plot
#' with MESOR, amplitude(s), and acrophase(s).
#' @param ci Logical; Default `TRUE` computes and draws pointwise parametric
#' confidence bands for the fitted cosinor curve using the model covariance.
#' @param ci_level Confidence level for the pointwise bands, expressed in
#' numeric value between 0 and 1.
#' @param n Integer; number of points on the fine prediction grid used to draw
#' the fitted cosinor and confidence ribbon. (default to `400`)
#' @param point_size Numeric; plotting size for observed points.
#' (default: `0.5`)
#' @param title_extra Optional character string appended to the plot title for
#' extra context.
#' @param legend.position Position of the legend on the plot; default is
#' `"right"`. Other options include `"top"`, `"bottom"`, `"left"`, or a numeric
...
#'
#' @seealso
#' \code{\link[ggplot2]{ggplot}} \code{\link[ggrepel]{geom_label_repel}}
#'
#' @examples
#' \dontrun{
#' # Import data
...
ggCosinorM <- function(object, labels = TRUE, ci = TRUE, ci_level = 0.95,
  ...
  if (!inherits(object, c("CosinorM", "CosinorM.KDE"))) {
    stop("ggCosinorM: object must be a CosinorM or CosinorM.KDE fit.",
         call. = FALSE)
  }
  ...
  if (!is.null(object$model) && !is.null(object$model$time)) {
    ...
    stop("Object missing model$time/activity for observed data.",
         call. = FALSE)
  }
  ...
  # title and axes ---------------------
  essential_tau <- ifelse(length(tau) == 1, as.character(tau),
                          paste(tau, collapse = ","))
  method_lbl <- ifelse(!is.null(object$method), object$method,
                       ifelse(inherits(object, "CosinorM.KDE"),"KDE",
                              "Parametric"))
