...
#'
#'
#' @details Transforms a continuous actigraphy time-series into a day-by-day
#' list of aligned epoch and produces a fully annotated data frame. Based on
#' the summary table generated by either \code{\link{BriefSum}} or
#' \code{\link{TAdjust}}, it re-organize and segment the raw recording data by
...
#'
#'
#' @returns A data list that includes the adjusted recordings both in long
#' format and as a nested list by day.
#'
#'
...
Act2Daily <- function (data, Bdf, TUnit = "hour", VAct = NULL, VTm = NULL,
  ...
  # Extract essential per-day metadata from Bdf -------------------
    ...
    if (is.null (VAct))
      VAct <- names (data) [[2]] # Default: second column of data
    if (is.null (VTm))
      VTm  <- names (data) [[1]] # Default: first column of data
    ...
    ## Identify which data columns were not generated by ActiGlobe ------------

    VNames <- names (data)
    VNames <- VNames [!VNames %in% c (
    ...
    for (d in seq_along (DT)) {
        ...
        # Extract the raw data rows for this day's epochs
        Temp   <- data [S:E, VNames]
        ...
        names (Temp) <- c (
            VNames, "DateTime", "Date", "Time", "UTC",
            "DaylightSaving", "nPoint", "Note"
        )
        Temp$DateTime <- paste0 (
            as.character (Temp$Date), " ",
            as.character (Temp$Time)
        )
    ...
    ndf <- data
    ndf$Note <- ""
    ndf$Note [oNP %in% nNP] <- df3.5$Note [nNP %in% oNP]

    VNames2 <- names (data) [names (data) %in% names (Temp)]
    ndf [ndf$nPoint %in% nNP, VNames2] <- df3.5 [VNames2]
    ndf [ndf$nPoint %in% mNP, "Note"]  <- "Unallocated"


    ## Return a list: per-day data and the full annotated data ----------------
