...
#'
#' @note
#' When you cross into a time zone with a larger UTC offset `(e.g., Montreal UTC–5 → Taipei UTC+8)`, it will generate a small overlap of epochs between the day
...
#'  summary table generated by either [BriefSum()] or [TAdjust()], it re-organize and
...
#'   please, use the updated version from \code{TAdjust}.
...
#'
#' @examples
#' # Option 1. Create a list that includes the adjusted recordings both in long
...
#' }
#'
#' # Option 2, separate storage with the help of "%<-%" operate from "zeallot"
#' \dontrun{
#' library(zeallot)
#' c(Dailydf, Newdf) %<-%
#'  Act2Daily(df = df,
