...
#'
#'
#' @details Transforms a continuous actigraphy time-series into a
#'  day-by-day list of aligned epoch and produces a fully annotated
#'  data frame. Based on the summary table generated by either
#'  \code{\link{BriefSum}} or \code{\link{TAdjust}}, it re-organize
#'  and segment the raw recording data by each day. The function is
#'  designed to process incomplete recordings while accounting for
#'  travel and daylight-saving adjustments. All labels contained in
#'  the summary table (i.e., Bdf) will be assigned to the
#'  corresponding data points. This workflow ensures that each day's
#'  recording is bounded by its calendar date while accounts for
#'  travel--induced overlaps, and clearly documents any incomplete or
#'  unallocated points.
#'
#'
...
#' @param VTm Optional character.  Name of the date.time index
#'   column in `df`. If NULL, defaults to the first column of `df`.
...
#'
#'
#' @returns A data list that includes the adjusted recordings both
#'   in long format and as a nested list by day.
#'
#'
