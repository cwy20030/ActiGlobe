...
#' @param VAct Optional character. Name of the activity column in each data.frame object (`df`) in `DailyAct`. If NULL,
#'   defaults to the second column of `df`.
#' @param VTm Optional character. Name of the activity column in each data.frame object (`df`) in `DailyAct`. If NULL,
...
#' }
#' @param tau Numeric scalar or vector for the assumed circadian period, passed to \code{\link{CosinorM}}. Default is 24 for single-phase; multiple phases can be supplied via [c()].
#' @param ph Optional logical scaler; if only a single period is assigned to tau, TRUE will force the function to append the post-hoc circadian estimates instead of the model estimates. (default: FALSE)
#' @param overwrite Logical; if TRUE, existing files with the same name will be overwritten. Default = FALSE.
#'
#' @returns Creates a multi-page `.pdf` visual document and a text-based `.csv` summary.
#' \itemize{
...
#' }
#'
#' @seealso \code{\link{Act2Daily}} \code{\link{CosinorM}} \code{\link{CosinorM.KDE}}
#'
#' @examples
#' \dontrun{
#'
