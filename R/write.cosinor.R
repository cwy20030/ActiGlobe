...
#' \code{\link{CosinorM}} or \code{\link{CosinorM.KDE}}. All daily
#' results could be found in the exported report. A summary file in
#' .csv format is also generated to update the original `Bdf` with
...
#'   each data.frame object (`df`) in `DailyAct`. If NULL, defaults
#'   to the second column of `df`.
#' @param VTm Optional character. Name of the activity column in each
#'   data.frame object (`df`) in `DailyAct`. If NULL, defaults to the
#'   first column of `df`.
...
#'   \item "FGLS": Feasible generalized least squares cosinor model
#'     via \code{\link{CosinorM}}.
#'   \item "KDE": Gaussian kernel density estimate (KDE) on circular
#'     time (hours of day) via \code{\link{CosinorM.KDE}}.
#' }
#' @param tau Numeric scalar or vector for the assumed circadian
#'   period, passed to \code{\link{CosinorM}}. Default is 24 for
#'   single-phase; multiple phases can be supplied via [c()].
#' @param ph Optional logical scaler; if only a single period is
#'   assigned to tau, TRUE will force the function to append the
#'   post-hoc circadian estimates instead of the model estimates.
#'   (default: FALSE)
#' @param overwrite Logical; if TRUE, existing files with the same
#'   name will be overwritten. Default = FALSE.
#'
#' @returns Creates a multi-page `.pdf` visual document and a
#'   text-based `.csv` summary.
#' \itemize{
#' \item PDF  A multi-page PDF written to `pdfDir` (one file per
#'   `ID`). Each page corresponds to a recording day and contains two
#'   panels: the top panel shows the raw activity (points) and a
#'   smoothed trend; the bottom panel shows the model fit for the
#'   same day
#' \item `Summary.csv` An updated `Bdf` documentation with one row
#'   per recording day. Model-based circadian parameters are appened
#'   at the end of the file for each day.
#' }
#'
#' @seealso \code{\link{Act2Daily}} \code{\link{CosinorM}}
...
write.cosinor <- function(Dir, ID, DailyAct, Bdf, VAct = NULL, VTm = NULL,
  ...
  if (all(length(tau) == 1, isFALSE(ph))) {
    ...
    nVNames <- c("MESOR", "Bathyphase.time", "Trough.ph", "Acrophase.time",
                 "Peak", "Amplitude")
    Bdf[nVNames] <- NA
