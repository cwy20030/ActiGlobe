...
#'
#' @description
#' This function fits a generalized linear model (GLM) to observed activity
#' data in order to estimate periods of inactivity. A polynomial term of time
...
Prob.Inact <- function(y, time, k = 12, threshold = 3, logical = TRUE) {
  ...
  pred <- predict(fit_glm, newdata = data.frame(Time = time),
