...
#'
#' @details
#' The `Cosinor` model is a cosine-based harmonic regression used
#' to estimate circadian rhythm parameters.
#'
#' \strong{Single-phase equation}:
#'       \deqn{y = M + A \cos \left(\frac{2\pi t}{\tau} - \phi\right) }
#' \itemize{
#'   \item \eqn{M }: MESOR (mid-line estimating statistic of
#'     rhythm), the intercept
...
#'   \item \eqn{\phi }: Acrophase (time-of-peak), computed from
#'     fitted sine and cosine coefficients
#' }
#'
#'
...
#' \itemize{
#'   \item \eqn{\beta = A * cos(\phi)}, estimated coefficient for
#'     the cosine term
...
#'  Note,
#' \itemize{
#'   \item \eqn{\phi } can be converted to clock time to identify
#'     the peak activity time AFTER midnight.
#'   \item if the model is not a mono-phase (i.e., more than one
#'     \eqn{\tau }), there will be \eqn{\frac{24 (hours)} {\tau} }
...
#' @param tau Numeric scalar or vector for the assumed circadian
...
#'   \item "FGLS": Feasible generalized least squares; models
...
#' @param type Character string passed to
#'   \code{\link[sandwich]{vcovHC}} for robust standard error
#'   computation
#' @param dilute Logical;
#' \itemize{
#'   \item "FALSE": All essential parameters would be produced.  (default)
#'   \item "TRUE": Only cosinor coefficients are returned. This is
...
#'     Amplitude.ph)
#'   \item extra: only available for ultradian (i.e., \eqn{\tau} less
#'     than 24hour) single-component model
...
#' 22, 392-399. doi:10.2307/2346786.
#'
#' Harvey, A. C. (1976). Estimating Regression Models with
...
### start or the end time falls are not the beginning/last time point of the
### recording.
