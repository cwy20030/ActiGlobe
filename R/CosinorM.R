...
#'
#' @details
#' The `Cosinor` model is a cosine-based harmonic regression used to estimate
#' circadian rhythm parameters.
#'
#' \strong{Single-phase equation}:
#'       \deqn{y = M + A \cos \left(\frac{2\pi t}{\tau} - \phi\right) }
#' \itemize{
#'   \item \eqn{M }: MESOR (mid-line estimating statistic of rhythm), the
#'   intercept
...
#'   \item \eqn{\phi }: Acrophase (time-of-peak), computed from fitted sine and
#'   cosine coefficients
#' }
#'
#'
...
#' Default is 24 for single-phase; multiple phases can be supplied via [c()]
...
#'   \item extra: only available for ultradian (i.e., \eqn{\tau} less than
#'   24hour) single-component model
...
#'
#' Chambers, J. M. (1992) Linear models. Chapter 4 of Statistical Models in S
