...
```{r echo=FALSE, message=FALSE, warning=FALSE, tab.cap="Table 2 The Header of Bdf", fig.width=8}
knitr::kable (head (Bdf), format = "html", table.attr = "style='width:800px;'")
```

In the enriched data - `data`, we have both the original data stored in `FlyEast` and some new information, such as the time stamp of each data point.

```{r View data, message=FALSE, warning=FALSE, results='hide'}
data <- BdfList$data
...
knitr::kable (head (data), format = "html", table.attr = "style='width:800px;'")
```
...
Fortunately, we already created something similar for this tutorial - TLog. The `TLog` was created using the travel diary template, which can be created using the function `TravelLog()`. For more information on the standard travel diary, type `?TravelLog` or `help (TravelLog)` in the R console.
...
knitr::kable (head (IANA), format = "html", 
              table.attr = "style='width:700px;'")
```
## Step 2: Adjust time change based on the travel diary
###  {.scrollable}
Before `ActiGlobe` directly modify information stored in the enriched longitudinal recording - `data`, we will need to adjust the brief summary stored in `Bdf`. This design allows us to quickly scan through the summary file, just in case any adjustment is not properly addressed.

```{r TAdjust_Action, echo = FALSE, message=FALSE, warning=FALSE, results='hide'}
...
```{r echo = FALSE, message=FALSE, warning=FALSE, paged.print=TRUE,tab.cap="Table 6 Initial Brief Summary of the Recording"}
knitr::kable (Bdf [10:15, ], format = "html", 
              table.attr = "style='width:700px;'") 
```
...
```{r echo = FALSE, message=FALSE, warning=FALSE, paged.print=TRUE,tab.cap="Table 7. Adjusted Brief Summary of the Recording"}
knitr::kable (Bdf.adj [10:15, ], format = "html", 
              table.attr = "style='width:700px;'") 
```
