...
df$theta <- (C2T(df$Time)%% 24) * 2 * pi / 24
...
Model <- card::cosinor(Activity ~ hour, data = df, tau = 24)
Model

card::ggcosinor(Model)
```



```{r}
cosinor2::correct.acrophase(cosinor::cosinor.lm(Activity ~ time(hour),data = df, period = 24))
```
...
```{r 6 12 hour OLS, echo=FALSE, fig.height=5, fig.width=7, message=FALSE, warning=FALSE}
for (i in c(6,12)){
